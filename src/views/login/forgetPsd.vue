<template>
	<div>
        <headerpage
      :title_page="title_page='找回密码'"
      :backBtn="backBtn=1"
    ></headerpage>
		<form class="form-login" @submit.prevent="formSubmit">
			<label for="userName">ID/手机号</label>
			<div class="form-row">
				<input id="userName" name="username" focus class="login-input" type="text" v-model="userName" placeholder="请输入ID/手机号" />
				<img src="../../../static/close.png" mode="widthFix" @click="closeInput">
			</div>
			<label for="">验证码</label>
			<div class="form-row">
				<input class="login-input" v-model="verifCode" name="verifCode" type="text" value="" placeholder="验证码(1分钟内有效)" />
				<img class="verif-code" src="../../../static/verif-code.png" @click="sendCode">
			</div>
			<label for="">新密码</label>
			<div class="form-row">
				<input class="login-input" v-model="password" name="password" :type="showType?'text':'password'" placeholder="至少6位数" />
  ...
  methods: {
    ...
    sendCode() {
      ...
      this.myAjax.noTokenPost(
        "user/sendSms?mobile=" + this.userName,
        function(result) {
